...
import { 
  ...
  List, ListOrdered, Type, CheckSquare, Minus 
} from "lucide-react";
...
export const commands: SlashCommand[] = [
  ...
  { name: 'Heading 2', action: 'heading2', icon: Heading2, description: 'Medium section heading.' },
  { name: 'Heading 3', action: 'heading3', icon: Heading3, description: 'Small section heading.' },
  { name: 'Bulleted List', action: 'bullet', icon: List, description: 'Create a simple bulleted list.' },
  { name: 'Numbered List', action: 'numbered', icon: ListOrdered, description: 'Create a list with numbering.' },
  { name: 'To-do List', action: 'todo', icon: CheckSquare, description: 'Track tasks with a to-do list.' },
  ...
  { name: 'Strikethrough', action: 'strikethrough', icon: Strikethrough, description: 'Cross out text.' },
    ...
    >
      ...
      {props.items.map((item, index) => {
        ...
        return (
          ...
          >
            <div className={`mr-3 p-1.5 rounded-md ${isSelected ? 'bg-gray-600' : 'bg-gray-700/50'}`}>
              <Icon className={`h-5 w-5 ${isSelected ? 'text-white' : 'text-gray-300'}`} />
            </div>
            <div>
              <div className={`font-medium ${isSelected ? 'text-white' : 'text-gray-200'}`}>{item.name}</div>
              <div className={`text-sm ${isSelected ? 'text-gray-300' : 'text-gray-400'}`}>{item.description}</div>
            </div>
          </button>
        );
      })}
