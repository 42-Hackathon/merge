...
const SlashCommandExtension = Extension.create({
  ...
  addProseMirrorPlugins() {
    const suggestionOptions: ExtendedSuggestionOptions = {
      ...
      items: ({ query }) => {
        return allCommands.filter(item => item.name.toLowerCase().startsWith(query.toLowerCase())).slice(0, 10)
      },
      render: () => {
        let component: any
      ...
      shouldRender: ({ state }) => {
        ...
        // Don't render if the cursor is in the first node
        return state.selection.from > firstNode.nodeSize;
      },
